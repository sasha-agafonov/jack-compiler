...
int IsClass(char* class) {
  for (int i = 0; i < classcnt; i++) if (strcmp(global[i].id, class) == 0) return 1;
  return 0; }


  Token GetVarKindBySubId(char* class, char* subscope, char* var) {
    Token tok;
    for (int i = 0; i < classcnt; i++) {
      if (strcmp(global[i].id, class) == 0) {
      for (int j = 0; j < global[i].count; j++) {
        if (strcmp(global[i].scope[j].id, subscope) == 0) {
          for (int k = 0; global[i].scope[j].count; k++) {
            if (strcmp(global[i].scope[j].scope[k].id, var) == 0) {
...
Token GetClassByVar(char* class, char* var) {
  Token tok;
  for (int i = 0; i < classcnt; i++) {
    if (strcmp(global[i].id, class) == 0) {
      for (int j = 0; j < global[i].count; j++) {
        if ((strcmp(global[i].scope[j].id, var) == 0) && ((strcmp(global[i].scope[j].kind, "static") == 0) || (strcmp(global[i].scope[j].kind, "field") == 0))) {
...
int GetVarOffset(char* class, char* var) {
  for (int i = 0; i < classcnt; i++) {
    if (strcmp(global[i].id, class) == 0) {
      for (int j = 0; j < global[i].count; j++) {
        for (int k = 0; k < global[i].scope[j].count; k++) {
...
Token Hunt(void) {
  Token tictac;
  tictac.tp = ERR;
  for (int j = 0; j < tekken_count; j++) if (LookupClass(tekken[j]) == 0) return tekken[j];
  return tictac; }
...
Token GetClassNameByVarId(char* var) {
  Token tok;
  for (int i = 0; i < classcnt; i++) {
    for (int j = 0; j < global[i].count; j++) {
      if (strcmp(global[i].scope[j].id, var) == 0) {
        strcpy(tok.lx, global[i].scope[j].type);
        return tok; } } }
  tok.tp = ERR;
  return tok; }
...
void InsertSubroutineScope(Token name, char* tp, char* kd) {
  if (classcnt == 0) return;
    ...
    global[classcnt - 1].scope[global[classcnt - 1].count - 1].scope
    = realloc(global[classcnt - 1].scope[global[classcnt - 1].count - 1].scope,
